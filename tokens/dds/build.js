...
const isRadiusToken = (token) =>
  ...
  token.path[token.path.length - 1] !== "smoothing" &&
  token.path[token.path.length - 1] !== "radiusType";
...
StyleDictionary.registerFilter({
  ...
  matcher: function (token) {
    return token.value !== "" && isBorderToken(token);
  },
});
  ...
  matcher: function (token) {
    return (
      ...
      (isFontToken(token) ||
        ...
        token.attributes.category === "$dds-innershadow" ||

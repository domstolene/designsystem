...
- f737169: Ny design token: `dds-color-border-strong`.

### Patch Changes

- 1ffefc9: Oppdaterer verdi i design token `dds-color-surface-paper` i mør kmodus i alle temaene.
- 1ffefc9: Oppdaterer verdi i design token `dds-color-brand-secondary-strong` i tema `supreme-dark`.
- 1ffefc9: Oppdaterer verdier i design tokens `dds-border-radius-button`, `dds-border-radius-surface`, `dds-border-radius-chip`, `dds-border-radius-input`, og `dds-border-radius-rounded` i temaene `supreme-dark` og `supreme-light`.
...
- dbd098d: Nye design tokens: motion tokens. Brukes til transitions og animasjoner, som fokusring, hover, accordion-effekt osv. Har prefiks `dds-motion`. Se [oversikten på Storybook](https://domstolene.github.io/designsystem/?path=/story/dds-design-tokens-tokens--motion).

import { Canvas, Controls, Meta } from '@storybook/addon-docs/blocks';
import { ComponentLinkRow } from '@norges-domstoler/storybook-components';
import * as EmptyContentStories from './EmptyContent.stories';

<Meta of={EmptyContentStories} />

# EmptyContent

<ComponentLinkRow
  docsHref="https://design.domstol.no/987b33f71/p/92d5ef-emptycontentview/b/007750"
  figmaHref="https://www.figma.com/file/ewqSDmkgqDQ5PyOsRp4V5b/DDS-Komponenter?node-id=12387%3A221195&t=ygT73thALrQ1LGjN-1"
  githubHref="https://github.com/domstolene/designsystem/tree/main/packages/dds-components/src/components/EmptyContent"
  storybookHref="https://domstolene.github.io/designsystem/?path=/story/dds-components-components-emptycontent--default"
/>

Komponenten brukes som en empty state når brukeren er forventet å velge innhold fra en liste eller lignende.

## Props

<Canvas of={EmptyContentStories.Preview} />
<Controls of={EmptyContentStories.Preview} />

## Eksempler

### I wrapper

<Canvas of={EmptyContentStories.InWrapper} />

### Langt innhold

<Canvas of={EmptyContentStories.LongContent} />

### Kompleks innhold

<Canvas of={EmptyContentStories.ComplexContent} />

import { Meta, Canvas, Controls } from '@storybook/addon-docs/blocks';
import { ComponentLinkRow } from '@norges-domstoler/storybook-components';
import * as InlineEditStories from './InlineEdit.stories';
import meta from './InlineEdit.stories';
import * as InlineEditInputStories from './InlineEditInput/InlineEditInput.stories';
import * as InlineEditTextAreaStories from './InlineEditTextArea/InlineEditTextArea.stories';
import * as InlineEditSelectStories from './InlineEditSelect/InlineEditSelect.stories';
import { Tabs, Tab, TabList, TabPanel, TabPanels } from '../Tabs';

<Meta of={meta} />

# InlineEdit

<ComponentLinkRow
  docsHref="767867-inlineedit"
  figmaHref="node-id=9005%3A150044"
  githubHref={{ folder: 'components', comp: 'InlineEdit' }}
  storybookHref={{ folder: 'components', comp: 'inlineedit' }}
/>

`<InlineEdit>`-komponentene håndterer inline redigering, der brukeren kan trykke `Enter` for å lagre verdien i feltet. Brukes f.eks i tabeller.

## Semantiske varianter

<Tabs>
  <TabList>
    <Tab>InlineEditInput</Tab>
    <Tab>InlineEditTextArea</Tab>
    <Tab>InlineEditSelect</Tab>
  </TabList>
  <TabPanels>
    <TabPanel>
      Inputfelt. Returnerer `<input>`.

      <Canvas of={InlineEditInputStories.Preview} />
      <Controls of={InlineEditInputStories.Preview} />
    </TabPanel>
    <TabPanel>
      Tekstområde. Returnerer `<textarea>`.

      <Canvas of={InlineEditTextAreaStories.Preview} />
      <Controls of={InlineEditTextAreaStories.Preview} />
    </TabPanel>
    <TabPanel>
      Nedtrekksliste. Returnerer nativ `<select>`. Bruk `<option>` som barn. **OBS!** husk tom `<option>` for initiell tom verdi og/eller tømbar variant. Komponenten tømmer verdi ved å velge `<option>` uten barn.

      <Canvas of={InlineEditSelectStories.Preview} />
      <Controls of={InlineEditSelectStories.Preview} />
    </TabPanel>

  </TabPanels>
</Tabs>

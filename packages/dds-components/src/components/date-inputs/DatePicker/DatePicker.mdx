import { Canvas, Controls, Meta, Story } from '@storybook/addon-docs/blocks';
import {
  Source,
  ComponentLinkRow,
} from '@norges-domstoler/storybook-components';
import * as DatePickerStories from './DatePicker.stories';

<Meta of={DatePickerStories} />

# DatePicker

<ComponentLinkRow
  docsHref="73c4d3-datepicker"
  figmaHref="node-id=24271-12545"
  githubHref={{ folder: 'components', comp: 'date-inputs/DatePicker' }}
  storybookHref={{ folder: 'components', comp: 'datepicker' }}
/>

`DatePicker`-komponenten er implementert vha. [react-aria (react-spectrum.adobe.com)](https://react-spectrum.adobe.com/react-aria).
For mer utfyllende dokumentasjon av props referrerer vi deg til [react-aria DatePicker (react-spectrum.adobe.com)](https://react-spectrum.adobe.com/react-aria/DatePicker.html#props).

## Props

<Canvas>
  <Story of={DatePickerStories.Preview} />
</Canvas>
<Controls of={DatePickerStories.Preview} />

## Bruk

<Source
  code={`
import { useState } from 'react';
import { DatePicker } from '@norges-domstoler/dds-components';
import { CalendarDate } from '@internationalized/date';

const [value, setValue] = useState<CalendarDate | undefined>(
new CalendarDate(2023, 8, 28)
);

<DatePicker label="Velg dato" value={value} onChange={setValue} />
`} />

### Med native JS Date i stedet for @internationalized/date

Vi anbefaler å bruke `@internationalized/date` for å representere datoer, da den ikke er avhengig av brukers locale og tidssone.
Om du derimot allikevel skulle ha behov for å bruke native JS Date, så har vi laget funksjoner for å oversette mellom native JS Date og `@internationalized/date`:

<Source
  code={`
function nativeDateToCalendarDate(date: Date): CalendarDate;
function calendarDateToNativeDate(date: CalendarDate, time?: Time): Date;
`}
/>

Eksempel på bruk av native JS Date:

<Source code={`
import { useState } from 'react';
import { DatePicker, nativeDateToCalendarDate, calendarDateToNativeDate } from '@norges-domstoler/dds-datepicker';

const [value, setValue] = useState<Date>(new Date());

<DatePicker
  label="Velg dato"
  value={nativeDateToCalendarDate(value)}
  onChange={d => setValue(calendarDateToNativeDate(d))}
/>
`}/>

### `maxValue`/`minValue` vs `isDateUnavailable`

Bruk disse props som følger:

- `maxValue`/`minValue` når det er logisk at datoer ikke er tilgjengelig, slik at brukeren ikke kan bla i de i kalenderen. Eksempler: registrere dødsdato kan ikke gjøres frem i tid, timebestilling skal ikke gjøres tilbake i tid.
- `isDateUnavailable`når begrensningen er vilkårlig. Brukeren skal kunne bla i datoene selv om de er utilgjengelige. Eksempler: ikke helg, ikke dato før vilkårlig tidspunkt.

### `maxValue`/`minValue` validering

Brukeren kan ikke velge datoer utenfor rekkevidde i kalenderen, men de kan fortsatt skrive inn ugyldig dato.
Dato utenfor rekkevidde må dermed valideres av konsumenten.

### Liten skjerm

Du kan sette brekkpunkt for når versjonen for mindre skjermer skal vises.

<Canvas of={DatePickerStories.Responsive} />

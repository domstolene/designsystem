...
<ComponentLinkRow
  docsHref="0822fe-card"
  figmaHref="node-id=3421%3A94375"
  githubHref={{ folder: 'components', comp: 'Card' }}
  storybookHref={{ folder: 'components', comp: 'card' }}
/>
...
`<Card>` får typen via `cardType`-prop.
...
<Tabs>
  <TabList>
    <Tab>Card</Tab>
    <Tab>CardExpandable</Tab>
    <Tab>CardExpandableHeader</Tab>
    <Tab>CardExpandableBody</Tab>
    <Tab>CardSelectable</Tab>
    <Tab>CardSelectableGroup</Tab>
  </TabList>

  <TabPanels>
    <TabPanel>
      En wrapper for innhold i et kort. Bruk design-tokens for spacing og annet styling der relevant. Har default typografi.

      <Canvas of={CardStories.Preview} />
      <Controls of={CardStories.Preview} />
    </TabPanel>
    <TabPanel>
      En container for CardExpandable-delkomponenter som håndterer oppførsel og universell utforming. Den skal ha:
    ...
    <TabPanel>
      Egen komponent for selectable variant. Returnerer radioknapp eller avkrysningboks.

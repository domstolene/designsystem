...
<ComponentLinkRow
  ...
  storybookHref={{ folder: 'patterns', comp: 'form--form' }}
/>
...
- **Spacing**: komponenter som `<FieldsetGroup>` har innbakt spacing som følger retningslinjer. Typografikomponenter bør brukes med `withMargins` og `withMarginsOverInput` prop. Sett i tillegg egen spacing for generiske containere; bruk gjerne [layout primitives](#layout-primitives).
...
<Tabs>
  ...
  <TabPanels paddingBlock='var(--dds-spacing-x1-5) 0'>
    ...
    <TabPanel>
      <Canvas of={FormStories.TwoButtons} />
    </TabPanel>
    <TabPanel>
      <Canvas of={FormStories.MultipleButtons} />
    </TabPanel>
    <TabPanel>

...
import { containerTokens, inputTokens } from './checkboxTestTokens';
...
const StyledCheckboxTest = styled.label<{ disabled?: boolean }>`
  display: inline-flex;
  align-items: center;
  position: relative;
  user-select: none;
...
export const CheckboxTest = forwardRef<HTMLInputElement, CheckboxTestProps>(
  ({ id, label, disabled, className, style, ...rest }, ref) => {
    const [uniqueId] = useState<string>(id ?? `checkboxTest-${nextUniqueId++}`);

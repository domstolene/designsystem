import {
  Meta,
  ArgsTable,
  PRIMARY_STORY,
  Story,
  Canvas
} from '@storybook/addon-docs';
import { TextInput } from '.';
import { Source, ComponentLinkRow, SB_DESIGNSYSTEM_URL } from '../../storybook';
import { Typography } from '../Typography';

<Meta title="Design system/TextInput/API" component={TextInput} />

# TextInput

<ComponentLinkRow
  docsHref="https://design.domstol.no/987b33f71/p/8048eb-textinput/b/800765"
  figmaHref="https://www.figma.com/file/ewqSDmkgqDQ5PyOsRp4V5b/?node-id=155%3A38"
  githubHref="https://github.com/domstolene/designsystem/tree/master/components/src/components/TextInput"
/>

## Demo

<Canvas>
  <Story id="design-system-textinput--input-with-label" />
</Canvas>
<Canvas>
  <Story id="design-system-textinput--default" />
</Canvas>

Se stories med kontrollere i <Typography externalLink typographyType="a" href={`${SB_DESIGNSYSTEM_URL}-textinput`}>Storybook</Typography>.

## Bruk i koden

<Source code={`import { TextInput } from '@norges-domstoler/dds-components';

<TextInput label="Ledetekst" />
<TextInput label="Ledetekst" multiline />
`} />

## API

<ArgsTable story={PRIMARY_STORY} />
I tillegg støttes alle native HTML-attributter som er en del av `InputHTMLAttributes<HTMLInputElement>` eller `InputHTMLAttributes<HTMLTextAreaElement>`-interface.

### maxLength

Når native attributtet `maxLength` brukes dukker det opp en tegnteller til høyre under inputfeltet:

<Canvas>
  <Story id="design-system-textinput--with-character-count" />
</Canvas>

## Retningslinjer

- Bruk gjerne `autofocus` for å indikere hvor brukeren skal starte.
- Unngå å bruke `placeholder` - inputfeltet kan se ferdig fylt ut.
- Bruk `onChange` slik at brukeren kan få umiddelbar tilbakemelding på deres input baser på validering.
- Det kan være lurt å lagre kladd av input i skjemaet brukeren fyller ut.
